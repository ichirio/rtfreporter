...
A token longer than its column keeps its own line rather than being cut
mid-word: a subject id or a lab code broken in half is worse than one that
overflows, and the column has a relative width in the rendered table, so RTF
wraps whatever does not fit.
...
cell, so this article's claim is checked rather than asserted. Three
differences are deliberate:

* **A token longer than the width keeps its own line.** The hand-written rule
  pushed an empty accumulator first, so such a cell opened with a blank line
  and the record was a row taller than it needed to be.

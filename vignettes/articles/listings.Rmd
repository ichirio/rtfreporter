...
exactly as written. Leave it out and the header is **derived from the data**:
each source column's `label` attribute when it has one, otherwise its name,
joined with the separator and wrapped to the column. ADaM read through haven
already carries those labels, so most listings need not write a single header
by hand. `label = ""` asks for a deliberately empty one.
...
separator -- for a column like `AGE/SEX`, which would otherwise spend two rows
on four characters -- and `collapse_repeats = TRUE` marks a **key** column,
printed once per record and again at the top of the next page.
...
Read the budget line first. The page's writable width, divided by the width of
one character in the listing's font, is how many characters there are to
spend; the gutters come out of it, because they print too.
...
from the listing's defaults is written out, so it reads like something a person
wrote.
...
returns one element per line -- the contract is in `?listing_spec`. There are
...
`listing_wrap_code()` writes the shipped rule out as source to paste into your
program and edit. It is the real code -- generated from the rule itself and
checked against it by the test suite, so it cannot disagree with what the
package runs. What it hands you is the *policy*: the three functions a fork
rewrites. The measurements they are built on -- `listing_disp_width()`,
`listing_take()`, `listing_split_after()` -- are exported and called, not
copied, so a fix to those reaches every fork:
...
* `?listing_col`, `?listing_spec`, `?build_listing` -- every setting, one page
  each.
* `?fit_listing_widths`, `?listing_code` -- the widths, and the source they
  come back as.

...
## 3. Two levels at once – `page_by`

`group_col` says where a **group** boundary is; `page_by` says where a
**page** boundary is, and what the page is called. Different jobs, so
different arguments – and together they express the layout a clinical
table usually wants: a BY variable (period, cohort, analysis set) owns
the page, while an inner group (a parameter block) is kept whole
*inside* it.
...
as_rtftables(
  lab,
  page_by   = "period",      # new page when it changes; the value is the name
  split     = "group_safe",  # from here on: inside one period only
  group_by  = "indent",
  max_rows  = 20,
  drop_cols = "period"       # page_by does not imply dropping the column
)
#>  "Period 1"  "Period 1"  "Period 2"  "Period 2"  "Period 3"
```

The body is partitioned on runs of the `page_by` value first; **every
other pagination setting then applies within one partition** – `split`,
`max_rows`, `group_col`, `group_by`, `min_group_rows`, `cont_label`,
`blank_rows`, `collapse_repeats`. `split = "by_value"` on its own cannot
do this: the group it protects is the BY column itself, so a parameter
block inside a period is cut wherever `max_rows` happens to fall.

Every page a partition makes carries the same **heading** – the
`page_by` value – because a page name is what
`rtf_tables(auto_section = TRUE)` / `auto_title = TRUE` prints. A
section opens where the heading **changes**, so one partition is one
section of however many pages it needed, with nothing to clean up.

The names themselves stay **unique**: a heading used by several pages
carries a `"...n"` tail, so the list can still be addressed by name (R
returns the *first* match for a repeated name – which is what
`pages[["Period 1"]]` and RStudio’s
[`View()`](https://rdrr.io/r/utils/View.html) would otherwise show you).
The tail is not part of the heading.

``` r

pages <- as_rtftables(lab, page_by = "period", split = "group_safe",
                      group_by = "indent", max_rows = 20, drop_cols = "period")
names(pages)
#>  "Period 1...1"  "Period 1...2"  "Period 2...1"  "Period 2...2"

doc |> rtf_tables(pages, auto_section = TRUE)   # 2 sections headed "Period 1" / "Period 2"
```

Two details worth knowing:

- `group_col` left `NULL` defaults to the first column **not** named in
  `page_by`. Without that it would be column 1 – the BY column, where
  every row of a partition carries the same value and
  `group_by = "indent"` finds no structure at all.
- `na` and `cell_format` run **body-wide, before** the partition, so one
  column width is shared by every page; `split_rows`, `blank_rows`
  positions and `count_blank_rows` are resolved **inside** each
  partition.

The three axes compose in one pipeline – pages by BY value, then rows,
then columns:

``` r

as_rtftables(lab, page_by = "period", split = "group_safe",
             group_by = "indent", max_rows = 20, drop_cols = "period") |>
  paginate_cols(at = 6, page_order = "down")
```

With `split = "by_value"` the split makes a page per **group** value,
and then the group is the **outer** axis with `page_by` inside it – a
group’s pages stay together, the BY values running in order, and every
page of the group carries the group’s name:

``` r

as_rtftables(ae, split = "by_value", group_col = "cohort", page_by = "period",
             drop_cols = c("cohort", "period"))
#>  "Cohort A"  "Cohort A"  "Cohort B"  "Cohort B"
```

The two values are never joined into one name: the name is the section
heading, so `"Cohort A / P1"` would cut each cohort into a section per
period. Both stay readable per page in
`attr(page$data, "rtf_paginate_meta")` (`page_group`, `page_by`).

Add
[`paginate_cols()`](https://ichirio.github.io/rtfreporter/reference/paginate_cols.md)
and the column block takes its place between them or under them, as
`page_order` says (see *Which axis the page number advances along*):
`"across"` gives group / column block / `page_by`, `"down"` gives group
/ `page_by` / column block.

## 4. Pre-splitting the data yourself
...
## 5. Advanced: post-processing a built-in split
...
result, so by default the **row page is the outer level**: a row band
sweeps every column block before the next band starts, so the reader
goes across the table first and then down it. Two row pages by three
column blocks come out as

    page 1  row 1 / cols 1    page 4  row 2 / cols 1
    page 2  row 1 / cols 2    page 5  row 2 / cols 2
    page 3  row 1 / cols 3    page 6  row 2 / cols 3

### Which axis the page number advances along – `page_order`

`page_order` says which axis the **page number advances along first**:
`"across"` (the default) advances **across the columns** – the column
block first, the row pages running inside it – and `"down"` advances
**down the rows** – the row page first, its column blocks following it.
...
```
    ...
    "across"                         "down"
    page 1  cols 1 / row 1           page 1  row 1 / cols 1
    page 2  cols 1 / row 2           page 2  row 1 / cols 2
    page 3  cols 2 / row 1           page 3  row 2 / cols 1
    page 4  cols 2 / row 2           page 4  row 2 / cols 2

The pages themselves are identical under either setting – only their
order differs – so pick the one your deliverable reads in: `"down"` when
a reader follows a row band across the sheet, `"across"` when each
column block is a self-contained table read on its own.

Those two are shorthands. In general `page_order` names the **axes** of
the split, in the order they nest – outermost first:

| axis | what it is |
|----|----|
| `"group"` | the pages a value-based split made (`as_rtftables(split = "by_value", group_col = )`) |
| `"rows"` | the row pages: `page_by`, and every other row split |
| `"cols"` | the column blocks [`paginate_cols()`](https://ichirio.github.io/rtfreporter/reference/paginate_cols.md) cuts |

``` r

paginate_cols(pages, at = c(4, 6), page_order = c("group", "cols", "rows"))
paginate_cols(pages, at = c(4, 6), page_order = c("cols", "group", "rows"))
paginate_cols(pages, at = c(4, 6), page_order = "cols")   # cols first, rest as usual
```

`"across"` is `c("group", "cols", "rows")` and `"down"` is
`c("group", "rows", "cols")`. An axis left out is appended in the
default order (`"group"`, `"rows"`, `"cols"`), and an axis the table
does not have never varies – so the same call works whether or not
`group_col` / `page_by` were used. Each page records its own group (in
`rtf_paginate_meta$page_group`), so the sort never has to read a page
name.

One consequence: `rtf_tables(auto_section = TRUE)` opens a section where
the page **name** changes, and a page is named by its group. A group’s
pages are one section only while `"group"` comes first; put `"cols"`
outside it and every column block starts the groups again, giving a
section per block and group.

One thing to know about the names: a page’s **name** follows the row
page it was cut from, so under `"across"` pages that share a name are no
longer adjacent – and that *does* reach the sections, because
`rtf_tables(auto_section = TRUE)` opens a section where the name
**changes**. `"down"` therefore keeps a table’s column pages in one
section, while `"across"` interleaves the names and gives a section per
page. An **unnamed** page list (what
[`as_rtftables()`](https://ichirio.github.io/rtfreporter/reference/as_rtftables.md)
returns for a single table) opens none at all. To put pages whose names
*differ* in one section, blank the ones that should not open a new one –
...
### A header whose values change per page – `set_col_header(values = )`

A header is usually one shape with a few values that change page to page
– `(N=120)` for one period, `(N=118)` for the next. Write it **once**,
with a `{token}` where a value belongs, and give
[`set_col_header()`](https://ichirio.github.io/rtfreporter/reference/set_col_header.md)
a table of values keyed by the page:
...
vals <- data.frame(
  group = c("Period 1", "Period 2", "Period 3"),
  n_pbo = c(120, 118, 238),
  n_trt = c(115, 112, 227)
)

as_rtftables(lab, split = "by_value", group_col = "period", ...) |>
  set_col_header(hdr, values = vals) |>
  paginate_cols(by = "____", carry = 1)
```

The key column is named after the **axis** it matches, the vocabulary
`page_order` uses: `group` (default – the value a `"by_value"` page was
cut for), `rows` (the `page_by` value) or `name` (the page’s unique
name); several may be combined. The keys live in the page’s metadata, so
they survive `drop_cols` and the column split.

Mistakes are errors, not silently wrong headers: a page with no row, a
row no page used, or a token with no value all stop the call and name
what is missing.
[`header_map()`](https://ichirio.github.io/rtfreporter/reference/header_map.md)
shows what every page ended up with:
...
(N=120)
n(%)"
#>  1    Period 1...1  Period 1 1    1   3    10   17 "TAK-003
(N=115)
n(%)"
```

### Where to cut, and what the header says – `by` / `col_header`

`at` and `cols` are positions. When the grouping is already **in the
column names** – the common wide layout, one column per treatment x
visit – `by` reads it from there: give it the separator, and the part
**before** it becomes the block key.
...
`col_header = "names"` then builds the two-level header those names
already carry – the key on top, the rest of the name below:

    page 1                            page 2
    |            |   Placebo     |    |            |   HOGE-001    |
    | Group      | D1 | D2 | D8  |    | Group      | D1 | D2 | D8  |

`by` also takes one **key per column** when the grouping is not in the
names.

A header written for the *whole* table goes in the same argument and is
sliced per page (spanning cells clipped):

``` r

paginate_cols(pages, at = c(11, 19), carry = 1, col_header = HDR)   # HDR = every label
```

Write the header **before or at** the split, never after it: a page
keeps only its own columns, so a full-table header no longer fits it,
and
[`set_col_header()`](https://ichirio.github.io/rtfreporter/reference/set_col_header.md)
/ `rtf_tables(col_header = )` say so rather than printing the first
...
Page names are carried through unchanged: each column page keeps the
name of the row page it was cut from (see *Which axis is the outer
level* above for what `auto_section` then does with them).
...
| One page per BY value, **and** an inner group kept whole | `page_by` + `split = "group_safe"` + `max_rows` |

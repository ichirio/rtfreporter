...
Names are carried **verbatim**, including ones that are not syntactic R
names. A gt column called `Drug A (N=60)` arrives as `Drug A (N=60)` –
spaces, parentheses and a leading digit and all – so select it with that
string, and backtick it where R needs a name rather than a string:
...
(Before v0.7.54 these were passed through
[`make.names()`](https://rdrr.io/r/base/make.names.html) and arrived as
`Drug.A..N.60.` / `X2024.total`, which is what this section always said
they did not do; see \#458.)
...
column by column. Pass a single formatter (applied to every data column;
the row-label column 1 is left alone) or a list of them taken
...
- [`fmt_count_paren_bare()`](https://ichirio.github.io/rtfreporter/reference/fmt_count_paren_bare.md)
  – the same, and a bare count with no parentheses (a lone `"0"`, a raw
  total) is padded into the same field;
- [`fmt_value_paren()`](https://ichirio.github.io/rtfreporter/reference/fmt_value_paren.md)
  – the general form: the value before the parenthesis may be **any
  text** (`"n=3"`, `"3/12"`, a word), the `(...)` block is
  right-justified the way
  [`format_count_pct()`](https://ichirio.github.io/rtfreporter/reference/format_count_pct.md)
  does it (the padding falls *before* the `(`, never inside the
  parentheses), and a zero count prints without its parentheses;
...
A formatter is either the **function** or its **name as a string**, with
the `fmt_` prefix optional – handy when the choice comes from a config
file or a recipe rather than from code:

``` r

as_rtftables(x, cell_format = "value_paren")
as_rtftables(x, cell_format = list(NULL, "right_align", my_fmt))
as_rtftables(x, align_count_pct = "count_paren")   # the switch takes a name too
```
...
[`fmt_value_paren()`](https://ichirio.github.io/rtfreporter/reference/fmt_value_paren.md)
is the one to reach for when a column mixes shapes – a plain N, a count
with a percentage, a zero, a text value:

``` r

mixed <- data.frame(
  Group = c("N", "Responders", "  Mild", "  Severe"),
  stat  = c("86", "12 (100)", "6 (50.0)", "0 (0.0)"),
  stringsAsFactors = FALSE
)
as_rtftable(mixed, cell_format = "value_paren")$data$stat
#> [1] "86       " "12  (100)" " 6 (50.0)" " 0       "
```

     86
     12  (100)      <- no decimals: flush against the ")"
      6 (50.0)
      0             <- a zero count prints on its own

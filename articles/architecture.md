...
- **Zero third-party runtime dependencies.** `Imports:` holds only
  packages that ship with R (`grDevices`, `grid`, `methods`, `utils`).
  Every optional integration (gt, gtsummary, rtables/tern, flextable,
  huxtable, ggplot2, …) lives in `Suggests:` and is reached through
  `.need_pkg()`, which errors helpfully when the package is absent and
  warns when it is too old.

...
`listing_spec(wrap = )` takes a function of `(text, width, sep, layout)`
that returns one element per line – the contract is in
[`?listing_spec`](https://ichirio.github.io/rtfreporter/reference/listing_spec.md).
There are two ways in, and they answer different questions.

**Adjust the rule from outside.**
[`listing_wrap()`](https://ichirio.github.io/rtfreporter/reference/listing_wrap.md)
*is* the shipped rule, so call it and fix up what comes back:

``` r

cap_two <- function(text, width, sep, layout) {
  out <- listing_wrap(text, width, sep, layout)
  if (!is.null(width) && length(out) > 2L) {
    out <- c(out[1L], paste0(substr(out[2L], 1, width - 1), "\u2026"))
  }
  out
}
...
**Change the rule from inside.** When the break logic itself has to
differ – a different notion of a word, a byte budget rather than a
display width –
[`listing_wrap_code()`](https://ichirio.github.io/rtfreporter/reference/listing_wrap_code.md)
writes the shipped rule out as source to paste into your program and
edit. It is the real code – generated from the rule itself and checked
against it by the test suite, so it cannot disagree with what the
package runs. What it hands you is the *policy*: the three functions a
fork rewrites. The measurements they are built on –
...
– are exported and called, not copied, so a fix to those reaches every
fork:
...
cat(head(src, 12), sep = "\n")
#> # The "multiline" wrapping rule from rtfreporter 0.8.0.9002, to edit.
#> #

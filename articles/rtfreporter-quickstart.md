# Clinical RTF Reporting with rtfreporter

## Overview

`rtfreporter` produces clinical-trial–style RTF reports (tables,
listings, figures) without depending on r2rtf or any other RTF library.
...
Each analyte occupies its own RTF section so the analyte name appears in
the section header, changing from page to page.
...
To present a figure together with a summary table, place them on
adjacent pages — both can share the same section header.
...
| `{BOOK_PAGE}` | A slot left empty here, filled by `assemble_rtf(book_page =)` |

Two numbers, two jobs. The **per-table** number belongs in the header
and must be static, so that binding the table into a compiled document
does not change it: `"Page {PAGE} of {TOTAL_PAGES}"` — within one
generated file these count that file’s table, and
[`assemble_rtf()`](https://ichirio.github.io/rtfreporter/reference/assemble_rtf.md)
leaves them alone. The **document-wide** number belongs in the footer
and must be dynamic, because only the reader knows it:
`"Page {AUTO_PAGE} of {AUTO_TOTAL_PAGES}"`.

For a single standalone table,
`"Page {AUTO_PAGE} of {AUTO_TOTAL_PAGES}"` alone is enough — the
document *is* the table.

### Both numbers on one page

Writing both from the start works, but on a standalone deliverable they
are the same number, so it prints twice. `{BOOK_PAGE}` reserves the
position instead:

``` r

# the deliverable -- the slot renders empty, keeping the row's height
footer = rtf_footer(rows = list(
...
Standalone, the header reads `Page 1 of 3` and that footer row is blank.
In the book the same page reads `Page 1 of 3` at the top and
`Page 2 of 6` at the bottom — and because the row was already there at
its full height, filling it cannot move anything. The other footer rows
are untouched; each deliverable keeps its own footer.

The slot may sit in any band — header, footer, title or footnote — and
there is no addressing to learn: wherever you write the token is where
the number goes.
...
# A border applies to whatever you attach it to.  Here: the whole table.
tbl_framed <- rtftable(df,
  border = rtf_border(top = TRUE, bottom = TRUE,
                      left = TRUE, right = TRUE,
                      inside_h = TRUE)
)

# Or one kind of row at a time.
tbl_custom <- rtftable(df, border = "none") |>
  style_zone(
    header   = rtf_border(top = TRUE, bottom = TRUE),
...
variable-change detection, and regex rule. Combine them in a
[`list()`](https://rdrr.io/r/base/list.html) and the positions are
unioned.
...
accept parallel `titles` and `footnotes` lists — one entry per page,
each a character vector whose elements become individual paragraph
lines. `""` produces a blank line; `NULL` falls back to the package
defaults (title = one blank centred line; no footnote).
...
left-aligned paragraphs with a top divider line on the first line. Both
inherit the document font size (one font per document, 9 pt by default).
...
builds a **table-style record** that bundles default borders, alignment,
padding and row-height. Pass the same record to many
[`rtftable()`](https://ichirio.github.io/rtfreporter/reference/rtftable.md)
calls and each table snapshots the defaults it needs at construction
time. To derive a variant, call
...
The S3 architecture is documented in
...
| `col_spec` | list | Per-column align / bold / italic / indent / **border** (`rtf_border`) / header\_\* |
| `border` | `"tfl"` / object | Outer-frame borders (now: top on first header row, bottom on last) |
| `blank_rows` | int / spec / list | Integer positions, [`blank_rows_by_change()`](https://ichirio.github.io/rtfreporter/reference/blank_rows_by_change.md), [`blank_rows_by_rule()`](https://ichirio.github.io/rtfreporter/reference/blank_rows_by_rule.md), or a list thereof |
...
| All [`rtftable()`](https://ichirio.github.io/rtfreporter/reference/rtftable.md) formatting args | — | Applied to bare `data.frame` items in the list |
...
- [The pipe API in
  depth](https://ichirio.github.io/rtfreporter/articles/rtfreporter-pipes.md)
  — every verb, in the order you call them
- [Page

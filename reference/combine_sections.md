...
`as_rtftables(x, split = "by_value", group_col = ...)` (or
`page_by = ...`) already names each page by the group value, so
`auto_section = TRUE` gives one section per group directly – as long as
each group fits on **one** page. A group that needs several comes back
with that same name on each of them, and a section opens where the name
**changes**, so the group is one section of several pages.

...
- wrap:
  ...
  The rule that breaks a cell – and a derived header – into lines.
  `NULL` (default) uses the `type`'s own rule, which is exactly
  [`listing_wrap()`](https://ichirio.github.io/rtfreporter/reference/listing_wrap.md):
  passing `wrap = listing_wrap` changes nothing.
  ...
  :   called **positionally** with four:
      `function(text, width, sep, layout)`.

  `text`

  :   a **length-1** character – one composed cell, or the column's
      labels already joined. Never a vector.

  `width`

  :   the column's `width`: a positive number, **or `NULL`** for a
      column that sets none. Handle `NULL` – it means "no limit".

  `sep`

  :   the effective separator, the column's own or the listing's.

  `layout`

  :   `"stack"` or `"flow"`, already resolved to one string.

  value

  :   a **non-empty character vector**, one element per line.
  ...
  cannot write a function out and says so in the code it emits. To
  change the rule from the inside rather than around it, start from its
  source:

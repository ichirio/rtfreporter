...
- pos:
  ...
  - a selector **function** of the data column names – see
    ...
    least one column, and the matched columns must be adjacent. Name and
    selector resolution (and the `start <= end` check for named ranges)
    happens when the header is attached to a table.
...
`pos` may finally be a **selector function** – see
[`col_key()`](https://ichirio.github.io/rtfreporter/reference/col_key.md).
It is called with the data column names when the header is attached to a
table and answers which columns the cell covers, so a spanning cell can
be written in terms of what it labels rather than of column numbers:
...
[`col_key()`](https://ichirio.github.io/rtfreporter/reference/col_key.md)
for selecting columns by a column-name segment.
...
#> <col_cell pos=col_key("Drug A") label="Drug A">
```

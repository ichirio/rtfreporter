...
This is what `align_count_pct = TRUE` applies, column by column, to
every character column but the first (see
[`as_rtftables()`](https://ichirio.github.io/rtfreporter/reference/as_rtftables.md));
it is also the built-in named `"count_pct"`. It is exported so it can be
applied directly to a data.frame column outside of any pagination
context.
